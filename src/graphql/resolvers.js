...
export const resolvers = {
  ...
  Mutation: {
    ...
    bulkRecordRemittances: async (_, { input }, { user }) => {
      requireRole(user, 'ADMIN');
      const { year, month, collectionCategory, collectionName, entries } = input;
      ...
      if (collectionName) {
        // Caller specified exactly which collection this is (required whenever
        // a category, like National Collections, contains more than one collection).
        targetCollection = matchingCollections.find(
          c => c.name.toLowerCase().trim() === collectionName.toLowerCase().trim()
        );
        if (!targetCollection) {
          throw new Error(`"${collectionName}" is not an active collection under "${collectionCategory}"`);
        }
      } else if (matchingCollections.length > 1) {
        throw new Error(
          `"${collectionCategory}" has ${matchingCollections.length} collections (${matchingCollections.map(c => c.name).join(', ')}). Specify collectionName.`
        );
      } else {
        targetCollection = matchingCollections[0];
      }
