...
if [ -z "$ADMIN_TOKEN" ]; then
  echo "Login failed — check ADMIN_EMAIL/ADMIN_PASSWORD above."
  exit 1
fi
echo "Logged in."
echo
...
monthly = [r for r in records if r['month'] != 0]
print(f'  Total records: {len(records)}  (annual/month=0: {len(annual)}, monthly: {len(monthly)})')
if annual:
    sources = {}
    for r in annual:
    ...
    print('    No annual (month=0) records for this year.')
"
  echo
done
