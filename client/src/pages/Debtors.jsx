...
export function Debtors() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">Debtors</h1><p className="text-sm text-muted-foreground mt-1">{debtors.filter(d => !d.isPaid).length} outstanding records for {YEAR}</p></div>
      <Card><CardContent className="pt-0">
        {!debtors.length ? <div className="py-12 text-center text-muted-foreground">No debtor records for {YEAR}</div> : (
          <div className="overflow-x-auto"><table className="w-full text-sm">
            <thead><tr className="border-b">{['Parish','Period','Expected','Actual','Balance','Status'].map(h => <th key={h} className="text-left py-3 px-4 font-medium text-muted-foreground">{h}</th>)}</tr></thead>
            <tbody>{debtors.map(d => (
              <tr key={d.id} className="border-b hover:bg-muted/30 transition-colors">
                ...
                <td className="py-3 px-4"><Badge variant={d.isPaid ? 'success' : 'destructive'}>{d.isPaid ? 'Paid' : 'Outstanding'}</Badge></td>
              </tr>
            ))}</tbody>
          </table></div>
        )}
