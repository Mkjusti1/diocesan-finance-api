...
import {
  ...
  Users, ShieldAlert, LogOut, Landmark, UploadCloud, X, KeyRound
} from 'lucide-react';
...
const navItems = [
  ...
  { to: '/profile', icon: KeyRound, label: 'Profile & Security', roles: ['ADMIN','BISHOP','PRIEST'] },
];
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 16px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '34px', height: '34px', borderRadius: '8px', backgroundColor: '#D3542A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Landmark size={16} color="white" strokeWidth={2} />
            </div>
            <div>
              <p style={{ color: 'white', fontWeight: 700, fontSize: '13px' }}>Diocesan Finance</p>
              <p style={{ color: '#C89B6E', fontSize: '11px' }}>Management System</p>
            </div>
          </div>
        ...
        <nav style={{ flex: 1, padding: '12px', overflowY: 'auto' }}>
          {allowed.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to} end={to === '/'} onClick={onClose}
              ...
              {({ isActive }) => <><Icon size={18} strokeWidth={isActive ? 2.5 : 2} />{label}</>}
            </NavLink>
          ))}
        </nav>
        ...
        <div style={{ padding: '16px' }}>
          ...
          <button onClick={() => { logout(); onClose(); }}
            style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%', padding: '10px 12px', borderRadius: '8px', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.5)', fontSize: '14px', fontWeight: 500 }}>
